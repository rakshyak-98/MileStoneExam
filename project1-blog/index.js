...
document.getElementsByName("icon-add")[0].addEventListener("click", () => showModal());
document.getElementsByName("icon-close")[0].addEventListener("click", () => hideModal());
document.querySelector(".modal form").addEventListener("submit", onSubmit);
// loads posts
...
function setupLocalStorage() {
  ...
  localStorage.setItem(
    CONST_VAL.PWSkills_Blog,
    JSON.stringify({ data: [], last_count: 0 })
  );
}
...
function onLoad() {
  const { data: blogs } = JSON.parse(
    localStorage.getItem(CONST_VAL.PWSkills_Blog)
  );
  blogs.forEach((el) => {
    document
      .getElementById("blog-container")
      .insertAdjacentHTML("beforeend", getPostDOMString(el));
  });
}
